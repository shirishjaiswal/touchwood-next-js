...
type EmailEditorProps = {
	...
	handleMailCodeChange: (data: string) => void;
...
function EmailEditor({
	...
	const handleSelect = (event: SelectableDropdownOnChangeEvent) => {
		const selectedOption = event.target.value?.[0]; // ✅ FIX: get first selected directly
		if (selectedOption) {
			...
			<div className="flex flex-col flex-1 gap-4 p-4">
				...
				<div className="relative">
					<textarea
						...
						required
						rows={25}
						onChange={(e) => handleMailCodeChange(e.target.value)}
					...
					<AiHelpBox
						data=""
						updateData={handleMailCodeChange}
						setLoading={() => {}}
					/>
				</div>
			</div>
